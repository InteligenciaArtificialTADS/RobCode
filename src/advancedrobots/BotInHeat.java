...
public class BotInHeat extends AdvancedRobot {
	...
	public void run() {
		setColors(Color.lightGray, Color.red, Color.white);
	...
	void doRadar() {
		if (enemy.none()) {
			// look around if we have no enemy or haven't seen him in awhile
	...
	// This is the WRONG WAY to switch directions!
	// (watch him hug the wall and other bots when he hits one)
	...
	void doGun() {
		...
		// distance = rate * time, solve for time, offset by current time
		long time = getTime() + (long)(enemy.getDistance() / bulletSpeed);
